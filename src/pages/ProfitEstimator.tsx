    ...
    <div className="mx-auto max-w-4xl animate-fade-in p-4 space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-4 glass-card p-6 h-max">
          <div>
            <label className="mb-1.5 block text-sm font-semibold font-display text-gray-700">{t('select_crop')}</label>
          ...
          <div>
            <label className="mb-1.5 block text-sm font-semibold font-display text-gray-700">{t('land_size')}</label>
            <input value={acres} onChange={(event) => setAcres(event.target.value)} className="glass-input w-full" type="number" min="0" step="0.1" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-semibold font-display text-gray-700">{language === 'te' ? 'ఎకరాకు మొత్తం ఖర్చు (₹)' : 'Total Cost Per Acre (₹)'}</label>
            <input value={costPerAcre} onChange={(event) => setCostPerAcre(event.target.value)} className="glass-input w-full" type="number" min="0" />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-semibold font-display text-gray-700">{language === 'te' ? 'మార్కెట్ ధర సర్దుబాటు (₹/క్వింటాల్)' : 'Market Price Adjustment (₹/Qtl)'}</label>
        ...
        <div className="glass-panel relative overflow-hidden bg-gradient-to-br from-emerald-50/80 to-green-100/80 p-6 shadow-md border-emerald-200/50">
          ...
          <div className="space-y-3 text-sm relative z-10 font-medium">
            <p className="flex items-center justify-between rounded-xl bg-white/60 backdrop-blur-sm p-3 shadow-sm border border-emerald-100/50"><span>{language === 'te' ? 'అంచనా ధర' : 'Estimated Price'}</span><strong className="text-base text-gray-900">₹{summary.adjustedPrice.toLocaleString()}</strong></p>
            <p className="flex items-center justify-between rounded-xl bg-white/60 backdrop-blur-sm p-3 shadow-sm border border-emerald-100/50"><span>{language === 'te' ? 'స్థూల ఆదాయం' : 'Gross Income'}</span><strong className="text-base text-gray-900">₹{summary.gross.toLocaleString()}</strong></p>
            <p className="flex items-center justify-between rounded-xl bg-white/60 backdrop-blur-sm p-3 shadow-sm border border-emerald-100/50"><span>{language === 'te' ? 'మొత్తం ఖర్చు' : 'Total Cost'}</span><strong className="text-base text-gray-900">₹{summary.totalCost.toLocaleString()}</strong></p>
            <div className="flex items-center justify-between rounded-xl bg-white/80 backdrop-blur-sm p-4 shadow-sm border border-emerald-200/60 my-2">
              <span className="font-bold font-display text-gray-800">{language === 'te' ? 'నికర లాభం' : 'Net Profit'}</span>
              <strong className={`text-xl font-extrabold ${summary.net >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>₹{summary.net.toLocaleString()}</strong>
            </div>
            <p className="flex items-center justify-between rounded-xl bg-white/60 backdrop-blur-sm p-3 shadow-sm border border-emerald-100/50"><span>ROI</span><strong className="text-base text-gray-900">{summary.roi.toFixed(1)}%</strong></p>
          </div>
